...
class SelfConsistencyTest(BaseConsistencyTest):
    ...
    def entailment_model(self):
        ...
        if self._entailment_model is None:
            from services.detection.hallucination.entropy.src.models.entailment_model import (
                EntailmentDeberta,
            )
    ...
    def _cluster_responses(self, responses: list[str]) -> list[int]:
        ...
        """
        from services.detection.hallucination.entropy.src.services.semantic_ids import (
            get_semantic_ids,
        )
